...
const Insert = () => {
  ...
  const validationSchema = Yup.object({
    ...
    completedDate: Yup.string(), // No need to require this initially
  ...
  const onSubmit = async (values) => {
    ...
    try {
      const response = await axios.post("http://localhost:4000/Satodo", values);
      toast.success(response.data.message || "Data inserted successfully!");
      navigate("/tododata");
    } catch (error) {
   ...
   <div>
            ...
            {values.completedDate && (
              <div className="text-lg font-semibold text-center">
                Time Taken: {calculateTimeTaken(values.createAt, values.completedDate)}
              </div>
            )}
